    ...
    public function dashboard(Request $request) {
        ...
        try {
            ...
            Session::put('user', Auth::User());
                ...
                $provider_approved = Provider::where('status', 'approved')->count();
                $provider_banned = Provider::where('status', 'banned')->count();
                $provider_unapproved = Provider::where('status', 'document')->count();
                $provider_onboarding = Provider::where('status', 'onboarding')->count();
                $provider_online = ProviderService::where('status', 'active')->count();
                $provider_offline = ProviderService::where('status', 'offline')->count();
                $provider_riding = ProviderService::where('status', 'riding')->count();
                $provider_hold = ProviderService::where('status', 'hold')->count();
                $provider_redflag = Provider::where('is_red_flag', '1')->count();
                $provider_orangeflag = Provider::where('is_orenge_flag', '1')->count();
            ...
            return view('admin.dashboard', compact('providers', 'provider', 'scheduled_rides', 'users', 'rides', 'totalRides', 'user_cancelled', 'provider_cancelled', 'cancel_rides', 'revenue', 'wallet','provider_approved','provider_banned','provider_unapproved','provider_onboarding','provider_approved','provider_online','provider_offline','provider_riding','provider_hold','provider_redflag','provider_orangeflag'));
        } catch (Exception $e) {
